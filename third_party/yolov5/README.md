...
By default the classification dataloader assigns indices based on alphabetical folder names. To keep a specific left/right-aware ordering, add a `classes.txt` (or `classes.names`) file in your dataset root (next to `train/`, `val/`, etc.) listing one class per line. The file order becomes the index→class map for every split. For example:
...
This produces the mapping `0 → arms_down`, `1 → both_arms_up`, `2 → left_arm_out`, `3 → right_arm_out`.
